# IdeaDeck
Landing page generator with focus on performance, accessibility and minimal design build with progressive enhancement principles. Project is build as part of 10kApart challenge. All pages are below 10 KB limit (including the three sample landing pages in gallery).
...
* Landing pages are compiled from a `jade` template that makes sure that they only contain the required `JavaScript` and `CSS`. - Carousel `JavaScript` and `CSS` are included only if user chooses to have more than one image in the landing page.
...
* `area-invalid` & `aria-describedby` used in form wherever necessary.
* Managed `autocorrection` and `spellcheck` in form using proper attributes.
...
* Generated landing pages with more than one image displays them in a `div` with horizontal scroll. If `JavaScript` is available, images are displayed as `carousel` with `next` and `previous` controls.
