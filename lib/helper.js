...
var fs = require('fs'),
	path = require('path'),
	pug = require('pug');
const compiledFunction = pug.compileFile(__dirname + '/../routes/template.jade');
	...
	saveModelOnDisk: function (req, res, next) {
		...
		if(typeof next === 'function') {
			fs.writeFile(path.join(saveDir, 'index.json'), JSON.stringify(req.model, null, 4), function(err) {
				...
				fs.writeFile(path.join(saveDir, 'index.html'), compiledFunction(req.model), function (err) {
					return next(err);
				});
			});
		} else {
			fs.writeFileSync(path.join(saveDir, 'index.json'), JSON.stringify(req.model, null, 4));
			fs.writeFileSync(path.join(saveDir, 'index.html'), compiledFunction(req.model));
		}
	}
};
